...
debug: build # build the Go binaries with debug symbols

test-ci: build
	find . -name "*_test.go" | awk -F/ '{NF=NF-1;$NF=$NF"/"}1' OFS=/ | grep -v '/build/' | xargs go test -v

model-files: # generate various model and model-topo YAML files in sdran-helm-charts/ran-simulator
...
# test: # run the unit tests and source code validation producing a golang style report
# test: build deps linters license
# 	go test -race github.com/nfvri/ran-simulator/...

# jenkins-test:  # run the unit tests and source code validation producing a junit style report for Jenkins
...
# publish: # publish version on github and dockerhub
# 	./build/build-tools/publish-version ${VERSION} nfvri/ran-simulator

# jenkins-publish: # Jenkins calls this to publish artifacts
